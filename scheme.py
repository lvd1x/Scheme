...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        expr = do_lambda_form(args, env)
        env.define(func_name, expr)
        return func_name
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    if_cond = scheme_eval(expressions.first, env)
    actions = expressions.second
    if scheme_true(if_cond):
        return scheme_eval(actions.first, env)
    elif scheme_false(if_cond) and actions.second is nil:
        return okay
    else:
        return scheme_eval(actions.second.first, env)
    # END Question 13
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    sub_expr = scheme_eval(expressions.first, env)
    if scheme_false(sub_expr):
        return False
